...
int main(int argc, char* argv[]){
  ...
  TF1 *g_calib = new TF1("calib","gaus",1800,2400);g_calib->SetLineColor(2);g_calib->SetLineWidth(2);
  ...
  int zedepnum=0;
  TFile *energyFile = new TFile(filename+"_energy.root","RECREATE");
  ...
  while(cbdInterface->numEvt()<entries){
    ...
    ss_evt<<i_evt;
    TString histoName="energy"+ss_evt.str();
    auto *energy2D = new TH2F(histoName,"energy;x;y",30,0,30,30,0,30);
    ...
    tEdep->Fill(Edep);
    {
      ...
      for (auto sipm:tower.SiPMs){
        ...
        energy2D->Fill(sipm.x,sipm.y,sipm.count*0.00031858);
        int hit = sipm.count;
        nhit+=hit;
        fired_sipm+=1;
